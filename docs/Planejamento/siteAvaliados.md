...
Principais problemas: Não há um link ou descrição clara de como se inscrever nos cursos.
...
Principais problemas: excesso de informação, gerando confusão a novos usuarios.
...
Principais problemas: Botão de gestão não funciona, so tem acesso a qualquer informação se logado. 
...
Principais problemas: Não existe menu, o usuario é direcionado para um mapa sem informaçõe de como ultilizar.
...
Principais problemas: Login não possui feedback, não é possivel saber qual é o problema, se a senha ou login esta incorreto ou se a conta não existe.
...
Principais problemas: So sem tem acesso a informações apos efetuar login.
